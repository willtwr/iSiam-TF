...
from datasets.transforms import Compose, RandomGray, RandomCrop, CenterCrop, RandomStretch, RandomFlip
from datasets.transforms import Resize, RandomBrightness, BboxCrop, RandomSaturation, RandomHue
from datasets.vid_reg import VID
  ...
  def __init__(self, config, is_training):
    ...
    if preprocess_name == 'siamese_fc_color':
      ...
      self.x_transform = Compose([RandomStretch(),
                                  ...
                                  RandomCrop(255 - 2 * 8),
                                  CenterCrop((188, 188)),
                                  Resize((255 - 2 * 8, 255 - 2 * 8))
                                  ])
    elif preprocess_name == 'None':
      self.v_transform = None
      self.z_transform = Compose([CenterCrop((100, 100)),
                                  Resize((127, 127))
                                  ])
      self.x_transform = Compose([CenterCrop((201, 201)),
                                  Resize((255, 255))
                                  ])
    else:
  ...
  def build_dataset(self):
    def sample_generator():
      for video_id in self.sampler:
        sample, ann = self.dataset_py[video_id]
        yield sample, ann

    def transform_fn(video, ann=None):
      ...
      if self.z_transform is not None:
        exemplar_image = self.z_transform(exemplar_image, tf.cast(ann[0][2::], tf.int32))
        wz = ann[0][2] / 100. * 127.
        hz = ann[0][3] / 100. * 127.
        
      if self.x_transform is not None:
        instance_image = self.x_transform(instance_image, tf.cast(ann[1][2::], tf.int32))
        wx = ann[1][2] / 100. * 127.
        hx = ann[1][3] / 100. * 127.
      
      return exemplar_image, instance_image, wz, hz, wx, hx

    dataset = tf.data.Dataset.from_generator(sample_generator,
                                             output_types=(tf.string, tf.float32),
                                             output_shapes=(tf.TensorShape([2]), tf.TensorShape([2, 4])))
