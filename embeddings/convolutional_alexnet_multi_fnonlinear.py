...
def convolutional_alexnet_arg_scope(embed_config,
  ...
  with slim.arg_scope(
      ...
      #activation_fn=lambda x: tf.nn.leaky_relu(x, alpha=0.0001),
...
def convolutional_alexnet(inputs, reuse=None, scope='convolutional_alexnet'):
  ...
  with tf.variable_scope(scope, 'convolutional_alexnet', [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.max_pool2d],
      ...
      nout2 = slim.conv2d(net, 256, [1, 1], 1, scope='conv2t')
      ...
      return nout5, nout2, end_points
    
    
def classifier(inputs, reuse=None, scope='classifier'):
  """Defines the feature extractor of SiamFC.
  ...
  Returns:
    ...
    end_points: the intermediate outputs of the embedding function.
  """
  with tf.variable_scope(scope, 'classifier', [inputs], reuse=reuse) as sc:
    end_points_collection = sc.name + '_end_points'
    with slim.arg_scope([slim.conv2d],
                        outputs_collections=end_points_collection):
      net = inputs
      net = slim.conv2d(net, 512, [1, 1], 1, scope='fc1')
      with slim.arg_scope([slim.conv2d],
                          activation_fn=None, normalizer_fn=None):
        nout = slim.conv2d(net, 2, [1, 1], 1, scope='fc2')
      # Convert end_points_collection into a dictionary of end_points.
      end_points = slim.utils.convert_collection_to_dict(end_points_collection)
      return nout
    
    
def convolutional_seg(inputs, reuse=None, scope='convolutional_seg'):
  """Defines the segmentation network.
  ...
  Returns:
    ...
    end_points: the intermediate outputs of the embedding function.
  """
  with tf.variable_scope(scope, 'convolutional_seg', [inputs], reuse=reuse) as sc:
    end_points_collection = sc.name + '_end_points'
    
    def _deconv(x, filters, ksize, stride=2, name='deconv', up_size=None):
      with tf.variable_scope(name, 'deconv'):
        if up_size is None:
          up_size = x.get_shape().as_list()[1:3]
          up_size = [up_size[0] * stride, up_size[1] * stride]
        x_up = tf.image.resize_images(x, up_size,
                                      method=tf.image.ResizeMethod.BICUBIC,
                                      align_corners=True)
        return slim.conv2d(x_up, filters, ksize, 1, scope='conv')
    
    with slim.arg_scope([slim.conv2d], padding='SAME', 
                        outputs_collections=end_points_collection):
      net = inputs  # 6 / 12
      
      net = slim.conv2d(net, 256, [3, 3], 1, scope='deconv0')
      net = _deconv(net, 128, [3, 3], 2, 'deconv1')  # 24
      net = _deconv(net, 64, [3, 3], 2, 'deconv2')  # 48
      
      #net = slim.conv2d(net, 256, [1, 1], 1, scope='deconv0')
      #net = _deconv(net, 256, [3, 3], 2, 'deconv1')  # 10
      #net = _deconv(net, 128, [3, 3], 2, 'deconv2')  # 18
      #net = _deconv(net, 64, [3, 3], 2, 'deconv3')  # 34
      
      #net = _deconv(net, 16, [3, 3], 2, 'deconv1')  # 10
      #net = _deconv(net, 32, [3, 3], 2, 'deconv2')  # 18
      #net = _deconv(net, 64, [3, 3], 2, 'deconv3')  # 34
      
      with slim.arg_scope([slim.conv2d],
                          activation_fn=None, normalizer_fn=None):      
        net = _deconv(net, 1, [5, 5], up_size=[64, 64], name='deconv4')  # 64
